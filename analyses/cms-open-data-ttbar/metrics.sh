...
exetime=$(grep execution ${logdir}/ttbar.out | awk '{print $3}')
throughput=$(grep processtime ${logdir}/ttbar.out | awk '{print $7}')
data=$(grep amount ${logdir}/ttbar.out | awk '{print $5}')
data2=$(awk "BEGIN {print($data*1000^2/1024^3)}")
datarate=$(awk "BEGIN {print($data / $exetime)}")
datarate2=$(awk "BEGIN {print($data*1000^2/1024^2 / $exetime)}")
...
echo "Amount of data: $data MB ($data2 GiB)"
echo "Data rate: $datarate MB/sec ($datarate2 MiB/sec)"
